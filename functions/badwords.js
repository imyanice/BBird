/*
    Swearing Checke made by ookamicodes on Github;
    But a little bit modified :)
*/

module.exports = async function swearingChecker(message, client) {
  const data = client.db.automod;
  const warnMSG = `:warning: | <@${message.author.id}> ne dit pas de gros mots !`;
  const muteRole = "879764931073949736";
  const muteCount = 5;
  const kickCount = 10;
  const banCount = 20;
  const userId = message.author.id;
  let warnCount = data.get(userId);
  ...
  if (foundInTextWord) {
    ...
    if (warnCount === kickCount) {
      message.channel.send(
        `Tu as **${warnCount}** warning(s)! | Action: Kické | Raison: ${kickCount} warnings.`
      );
      message.member.kick();
    }
    if (warnCount === banCount) {
      message.channel.send(
        `Tu as **${warnCount}** warning(s)! | Action: Bannis | Raison: ${banCount} warnings.`
      );
      message.member.ban();
    }
  }
